...
# Tabs for visualization and Comparison\n
tab1, tab2, tab3 = st.tabs(["2D Plot", "3D Plot", "Top Similarities"])
with tab3:
    st.subheader("🔍 Top 3 Similarity Scores Comparison")
    knn_df = pd.read_csv(BASE_DIR / "results" / "predictions.csv")
    azure_df = pd.read_csv(BASE_DIR / "results" / "predictions_enriched_with_top3_similarity.csv")
    ...
    merged = pd.merge(
        knn_df[["Filename", "TopSimilarities"]].rename(columns={"TopSimilarities": "KNN_TopSimilarities"}),
        azure_df[["Filename", "TopSimilarities"]].rename(columns={"TopSimilarities": "Azure_TopSimilarities"}),
